...
export default function ColdMail({ isOpen, onRequestClose, onInsert }) {
  const [mailTemplate, setMailTemplate] = useState('Search for List(s)');

  const handleInsert = () => {
    if (mailTemplate !== 'Search for List(s)') {
      onInsert("<strong>Leads from </strong><h4 class='color-pink'>" + mailTemplate + "</h4>"); // Call parent handler to insert node
